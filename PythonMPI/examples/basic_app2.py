"""basic_app2.py
    ...
    To run, start Matlab and type:
        ...
        MPI_Run('basic_app2',4,{})

    Or, to run a different machine type:

        MPI_Run('basic_app2',4,{'machine1' 'machine2' 'machine3' 'machine4'})
...
if (my_rank == leader):
    ...
    if UseGraphics:
        ...
        Hf_fig = plt.imshow(output, origin = 'lower',  extent = [0, 10, 0, 10])
    ...
    for i in range(N2):
        # Do not include the leader in data dealing
        dest = i%(comm_size-1)+1
        dest_tag = input_tag + i;
        dest_data = input[:,i]
        # Note: dest_data is sent as a 1-D array (not a column vector)
...
if (my_rank != leader):
    ...
    for i in range(N2): # Compute who the destination is for this message.
        ...
        if (my_rank == dest):
            ...
            # Add my_rank to the i_input vector
            # print('received i_input:')
            # print(i_input)
            i_input = i_input + my_rank
            # print('received i_input plus my_rank:')
            # print(i_input)
