...
def grid_global_ind(d):
    ...
    if dim==2:
        ...
        for i in range(grid_dims[0]):
            global_ind[str(i)] = dict()
            for j in range(grid_dims[1]):
                local_falls = get_local_falls(d.pitfalls, grid, grid[i,j])
                global_ind[str(i)][str(j)] = get_global_ind(local_falls, grid_dims)
    elif dim==3:
        # get local indices for each processor in the grid
        for i in range(grid_dims[0]):
            for j in range(grid_dims[1]):
                for k in range(grid_dims[2]):
                    ...
                    global_ind[str(i)][str(j)][str(k)] = get_global_ind(local_falls, grid_dims)
    elif dim==4:
        # get local indices for each processor in the grid
        for i in range(grid_dims[0]):
            for j in range(grid_dims[1]):
                for k in range(grid_dims[2]):
                    for m in range(grid_dims[3]):
                        ...
                        global_ind[str(i)][str(j)][str(k)][str(m)] = get_global_ind(local_falls, grid_dims)
    else:
