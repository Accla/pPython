"""basic_app6.py
    ...
    This example builds on basic_app4.py and basic_app5.py. 
    ...
    This example builds on basic_app5.py by adding a job queue 
    ...
    To run, start Matlab and type:
        ...
        MPI_Run('basic_app6',4,{})

    Or, to run a different machine type:

        MPI_Run('basic_app6',4,{'machine1' 'machine2' 'machine3' 'machine4'})

    Output will be piped into 4 files:

        PythonMPI/basic_app6.0.out
        PythonMPI/basic_app6.1.out
        PythonMPI/basic_app6.2.out
        PythonMPI/basic_app6.3.out
...
if (my_rank == leader):
    ...
    while not done:
        ...
        if (sendCounter <= N2) and (np.sum(workers_busy) < (worker_job_queue_depth * num_workers)):
            # determine which queues are not full    
            queue_deficit = workers_busy - worker_job_queue_depth
            worker_index = np.argsort(queue_deficit)
            
            # send to the worker with the emptiest queue (assuming leader = 0)
            dest = worker_index[0]+1
            ...
            workers_busy[dest-1] = workers_busy[dest-1] + 1
