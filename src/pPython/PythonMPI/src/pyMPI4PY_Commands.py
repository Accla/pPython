...
def pyMPI4PY_Commands(py_file,MPI_COMM_WORLD,**argv):
    """pyMPI4PY_Commands  -  Commands to launch a python script remotely.

    Usage:
    ------
    defscommands, unix_command = pyMPI4PY_Commands(py_file,MPI_COMM_WORLD)
    ...
    grep_env_command = 'env | egrep -i "PPYTHON_USE_MPI4PY|slurm|mpi"'+nl
    ...
    COM_EPPAC = True
    defsbase = 'PythonMPI/PythonMPI4PYdefs' 
    defsfile = defsbase + '.py'

    # Replace my_script_file with py_file basename (withoutt .py)
    # Define MPI4PY MPI rank environment variable
    # outfile = 'PythonMPI/' + py_file + '.${MPI4PY_RANK}.out'
    if USE_MPI4PY:
        outfile = 'PythonMPI/' + py_file + '.${OMPI_COMM_WORLD_RANK}.out'
    else:
        outfile = 'PythonMPI/' + py_file + '.${MPI_COMM_WORLD_RANK}.out'
    ...
    # rank = 0 -> it is not used inside gen_command for MPI4PY
    commands = gen_commands(py_file,python_mpi_path,0,machine,MPI_COMM_WORLD,COM_EPPAC)
    ...
    if machine == host: # Target is host.
        ...
        if type == 'pc':
            ...
            #     FORWARD_ERR = ' 2> >(awk '+q+'BEGIN{FS="\\n"}{print "Pid='+str(rank)+': " $1}'+q+' |tee -a '+errfile+' >&2)'
            # FORWARD_ERR = ' 2> >(awk '+q+'BEGIN{FS="\\n"}{print "Pid=" ENVIRON["MPI_COMM_WORLD_RANK"] ": " $1}'+q+' |tee -a '+errfile+' >&2)'
            FORWARD_ERR = ' 2> >(awk '+q+'BEGIN{FS="\\n"}{print "Pid=" ENVIRON["OMPI_COMM_WORLD_RANK"] ": " $1}'+q+' |tee -a '+errfile+' >&2)'
            # unix_command = python_command+FORWARD_ERR+' &'+nl+'touch PythonMPI/pid.'+machine+'.$!'+nl
            unix_command = grep_env_command + python_command+FORWARD_ERR+' &'+nl+'touch PythonMPI/pid.'+machine+'.$!'+nl
    if EPPAC or IMPLICIT_EPPAC:
        ...
        unix_command = '$TASKSET_CMD '+unix_command
