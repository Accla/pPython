...
# Case 1: 
print('\nCase 1: compare aggregation time between old and new agg with a Dmap object.\n')

X  = rand(N,N,map=Amap)

sync = oagg(zeros(1, Np, map=Amap))   # Synchronize start.
zero_clock = timer()
Xg1 = agg(X)
THA = timer()
print('Hierachical Agg() Time (sec) = %f'%(THA-zero_clock))

zero_clock = timer()
Xg2 = oagg(X)
TOA = timer()
print('Old Agg() Time (sec)         = %f'%(TOA-zero_clock))
