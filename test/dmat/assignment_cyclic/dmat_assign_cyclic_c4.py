...
Amap = 1                                 # Serial map.
if PARALLEL:
    Amap = Dmap([Npx,Npx],'c',range(Np))    # Parallel map.
    Bmap = Dmap([Npx,Npx],'b',range(Np))    # Parallel map.
...
A = rand(N1,N,map=Bmap)         # Create a distributed matrix.
GA = agg(A)
Z = zeros(N1,N,map=Amap)        # Create a distributed matrix.
Talloc = timer()
