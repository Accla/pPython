...
Amap = 1                                 # Serial map.
if PARALLEL:
    Amap = Dmap([Np,1,1],{},range(Np))    # Parallel map.
...
zero_clock = timer()
A = rand(N,N,N2,map=Amap)         # Create a distributed matrix.
B = zeros(N,N,N2,map=Bmap)         # Create a distributed matrix.
Talloc = timer()
