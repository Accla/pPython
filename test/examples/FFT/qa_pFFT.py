...
if PARALLEL:
    ...
    # Xmap = Dmap([2,int(Np/2)],{},range(Np))    # Parallel map.
...
# print('After firt transpose_grid(X) call, ID=1')
# save_result(X,Pid,1)

print('Before transpose_grid(X): X.shape')
print(X.shape)
X = transpose_grid(X)        # Redistribute along 1st dimension.
print('After transpose_grid(X): X.shape')
print(X.shape)
Xloc = local(X)
Tcomm = timer()-tic

# print('After firt transpose_grid(X) call, ID=2')
# save_result(X,Pid,2)
...
# print('After firt fft(Xloc) call, ID=3')
# save_result(X,Pid,3)
...
tic = timer()
print('Before calling Xloc = fft(Xloc,0)        # FFT 1st dimension.')
print('Xloc.shape')
print(Xloc.shape)
# print(Xloc)
# Xloc = fft(Xloc,0)        # FFT 1st dimension.
Xloc = fft(Xloc,len(Xloc),0)        # FFT 1st dimension.
X = put_local(X,Xloc)
Tcomp = Tcomp + timer()-tic

print('After calling Xloc = fft(Xloc,0)        # FFT 1st dimension.')
print('Xloc.shape')
print(Xloc.shape)
# print(Xloc)
...
if (Pid == 0):
    print('Xglobal')
    print(Xglobal.shape)
    np.save('test_Xglobal.npy', Xglobal)    # .npy extension is added if not given
    #To load: Xglobal = np.load('test3.npy')

if VALIDATE:
    myX = global_block_range(X,1)[0]     # Get global index.
    #MATLAB: iphase = phases + 1                   # Get phase index.
    iphase = phases                    # Get phase index.
    ...
    Err = put_local(zeros(1,Np,map=Xmap),Errloc)
    Erragg = agg(Err)
    if (Pid == 0):
